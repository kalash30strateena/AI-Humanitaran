...
def clean_and_rename_csv(download_dir, city_name):
    ...
    for csv_file in glob.glob(os.path.join(download_dir, "*.csv")):
        if os.path.basename(csv_file) == "grafico series temporalesserie t.csv":
            os.remove(csv_file)
    ...
    try:
        ...
        for batch_start in range(1, total_cities + 1, batch_size):
            ...
            for idx in range(batch_start, batch_end + 1):
                ...
                # Set "Meses" for Periodicidad de datos (first/default option)
                ...
                if not verify_csv_file(city_csv):
                    print(f"Downloaded CSV file has at least one null valor_indice for {city_name}, removing.")
                    os.remove(city_csv)
                    continue
